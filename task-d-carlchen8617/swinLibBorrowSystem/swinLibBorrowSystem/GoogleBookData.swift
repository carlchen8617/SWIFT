...
class GoogleBookData {
    ...
    func makeURL(isbn_id: String?){
        var  urlComponents = URLComponents(string: "https://www.googleapis.com/books/v1/volumes?")
        
        if isbn_id != nil {
            
            urlComponents?.query = "q=isbn:\(String(isbn_id!))&key=AIzaSyBEAdyWTrSGG2nVGL6ecqHUHK_w-AznTBI"
        }
        else{
            urlComponents?.query = "006265506X&key=AIzaSyBEAdyWTrSGG2nVGL6ecqHUHK_w-AznTBI"
        }
    ...
    /**
     ...
     // guard let url = URL(string:  "tabcorp-dcp-landed-sfmc-batch-1001-dev") else { return}
     var  urlComponents = URLComponents(string: "https://data.melbourne.vic.gov.au/resource/2biy-uyxu.json")
